    ...
    <div className="card shadow-sm border-0 mt-4">
      <div className="card-body">
        <div className="d-flex justify-content-between align-items-center mb-3">
          <div>
            <h5 className="card-title mb-0">Bulk import progress</h5>
            <small className="text-muted text-uppercase">
              {state.status === 'queued' && 'Queued'}
              {state.status === 'processing' && 'Processing'}
              {state.status === 'finished' && 'Completed'}
        ...
        <dl className="row mb-0">
          <dt className="col-sm-3">Successfully processed</dt>
          <dd className="col-sm-9">
            {state.processed} {state.total ? `of ${state.total}` : ''}
          </dd>
          <dt className="col-sm-3">Failed</dt>
        ...
        {state.errors && state.errors.length > 0 && (
          <div className="alert alert-warning mt-3" role="alert">
            <h6 className="alert-heading">Failed rows ({state.errors.length})</h6>
